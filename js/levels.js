...
function generateLevel(levelNumber) {
  ...
  const maxGap = 90 + difficulty * 120; // 90 ~ 210
  const spikeChance = levelNumber < 5 ? 0 : Math.min(0.4, 0.1 + difficulty * 0.4);
  const detourChance = levelNumber >= 12 ? Math.min(0.35, (levelNumber - 12) / 160) : 0;
  const movingChance = levelNumber >= 20 ? Math.min(0.45, (levelNumber - 20) / 140) : 0;

  const segCount = 6 + Math.floor(levelNumber / 8);
  ...
  for (let i = 0; i < segCount; i++) {
    ...
    if (roll < detourChance) {
      ...
      const platW = Math.max(70, 150 - difficulty * 80);
      const platY = C.GROUND_Y - (80 + rand() * 70);
      const platX1 = gapStart + gapLen / 2 - platW / 2;

      const isMoving = rand() < movingChance;
      platforms.push({
        x1: platX1,
        x2: platX1 + platW,
        y: platY,
        ...
        baseX1: platX1,
        baseX2: platX1 + platW,
        amplitude: isMoving ? 40 + rand() * 50 : 0,
        speed: isMoving ? 0.0012 + rand() * 0.0015 : 0,
      ...
      cursor += nextLen;
    } else if (roll < detourChance + 0.22 + difficulty * 0.15) {
      // 단순 구덩이(바닥 갭)
